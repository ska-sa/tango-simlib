...
class test_TangoSimGenDeviceIntegrationXMI(ClassCleanupUnittestMixin, unittest.TestCase):

    longMessage = True
    server_name = 'weather_ds'
    data_descr_file = 'Weather.xmi'
    sim_file_parser = sim_xmi_parser.XmiParser()
    ...
    def setUpClassWithCleanup(cls):
        ...
        cls.db_instance = helper_module.append_device_to_db_file(
            cls.server_name, server_instance, '%scontrol' % device_name,
    ...
    def setUp(self):
        super(test_TangoSimGenDeviceIntegrationXMI, self).setUp()
...
class test_TangoSimGenDeviceIntegrationFGO(test_TangoSimGenDeviceIntegrationXMI):

    @classmethod
    def setUpClassWithCleanup(cls):
        cls.server_name = 'databaseds'
        cls.data_descr_file = 'database2.fgo'
        super(test_TangoSimGenDeviceIntegrationFGO, cls).setUpClassWithCleanup()

    def setUp(self):
        self.sim_file_parser = fandango_json_parser.FandangoExportDeviceParser()
        super(test_TangoSimGenDeviceIntegrationFGO, self).setUp()
...
class test_TangoSimGenDeviceIntegrationSIMDD(test_TangoSimGenDeviceIntegrationXMI):

    @classmethod
    def setUpClassWithCleanup(cls):
        cls.server_name = 'weather_ds'
        cls.data_descr_file = 'Weather_SIMDD.json'
        super(test_TangoSimGenDeviceIntegrationSIMDD, cls).setUpClassWithCleanup()

    def setUp(self):
        self.sim_file_parser = simdd_json_parser.SimddParser()
        super(test_TangoSimGenDeviceIntegrationSIMDD, self).setUp()
    ...
    def test_device_command_list(self):
        ...
        simmdd json file
        """
        default_commands = helper_module.DEFAULT_TANGO_DEVICE_COMMANDS
        actual_device_commands = set(self.sim_device.get_command_list()) - default_commands
        expected_command_list = set(self.sim_file_parser.get_device_command_metadata().keys())
        self.assertEquals(actual_device_commands, expected_command_list,

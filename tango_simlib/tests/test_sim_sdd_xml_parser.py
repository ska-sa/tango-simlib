...
expected_pressure_mnt_pt_info = {
    ...
    "writable": None,
    "possiblevalues": [],
    "samplingfrequency": {"DefaultValue": None, "MaxValue": None},
    "logginglevel": None,
}
...
expected_on_cmd_info = {
    ...
    "cmd_params": None,
    "response_list": {
        "msg": {"ParameterName": "msg", "ParameterID": None, "ParameterValue": None},
        "gsm": {"ParamaterName": "gsm", "ParameterID": None, "ParameterValue": None},
    },
}
...
class test_Sdd_Xml_Parser(GenericSetup):
    def test_parsed_monitoring_points(self):
        ...
        expected_monitoring_points_list = [
            "Insolation",
            "Temperature",
            "Pressure",
            "Rainfall",
        ...
        self.assertEquals(
            set(expected_monitoring_points_list),
        ...
        for prop in expected_pressure_mnt_pt_info:
            self.assertEquals(
                actual_parsed_pressure_mnt_pt_info[prop],
    ...
    def test_model_quantities_metadata(self):
        ...
        for sim_quantity_name, sim_quantity in pmq.sim_model.sim_quantities.items():
            ...
            for mnt_pt_param_name, mnt_pt_param_val in mpt_meta.items():
                ...
                self.assertEqual(
                    sim_quantity_metadata[mnt_pt_param_name],
                    mnt_pt_param_val,
                    "The value of the param '%s' in the"
                    " model quantity '%s' is not the same with the one"
                    " captured in the SDD xml file for the monitoring"
                    " point '%s'."
                    % (mnt_pt_param_name, sim_quantity_name, mnt_pt_param_name),
                )

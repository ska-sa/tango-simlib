...
class test_DishElementMaster(ClassCleanupUnittestMixin, unittest.TestCase):
    ...
    def test_attribute_list(self):
        ...
        self.assertEqual(
            DISH_ELEMENT_MASTER_ATTRIBUTE_LIST,
            attributes,
    ...
    def test_low_power(self):
        ...
        for allowed_mode in ["STOW", "MAINTENANCE"]:
            ...
            self.assertEqual(
                power_state_quant.last_val, power_state_enum_labels.index("LOW")
            )
    ...
    def test_set_maintenance_mode(self):
        ...
        for allowed_mode in ["STANDBY-LP", "STANDBY-FP"]:
            ...
            self.assertEqual(
                dish_mode_quant.last_val, dish_mode_enum_labels.index("MAINTENANCE")
            )
    ...
    def test_set_standby_fp_mode(self):
        ...
        for not_allowed_mode in ["OFF", "STARTUP", "SHUTDOWN", "STANDBY-FP", "CONFIG"]:
            dish_mode_quant.last_val = dish_mode_enum_labels.index(not_allowed_mode)
    ...
    def test_set_stow_mode(self):
        ...
        self.assertEqual(
            pointing_state_quant.last_val, pointing_state_enum_labels.index("STOW")
        )
        ...
        with patch.object(self.model, "time_func") as mock_time:
            ...
            for update_x in sim_time_update:
                initial_actual_elevation = self.model.sim_quantities[
                    "achievedElevation"
                ].last_val
                self.model.update()
                self.assertGreater(
    ...
    def test_slew(self):
        ...
        dish_mode_quant.last_val = dish_mode_enum_labels.index("OPERATE")
        self.assertNotEqual(
            pointing_state_quant.last_val, pointing_state_enum_labels.index("SLEW")
        )
        dummy_timestamp = "31351.131"
        target_azim = 180.0
        target_elev = 90.0
        self.model.sim_actions["Slew"](
            data_input=[dummy_timestamp, target_azim, target_elev]
        )
        self.assertEqual(
            self.model.sim_quantities["desiredAzimuth"].last_val, target_azim
        )
        self.assertEqual(
            self.model.sim_quantities["desiredElevation"].last_val, target_elev
        )
        self.assertEqual(
            self.model.sim_quantities["desiredPointing"].last_val,
        ...
        with patch.object(self.model, "time_func") as mock_time:
            ...
            for update_x, expected_azim_position, expected_elev_position in zip(
                ...
                initial_actual_azimuth = self.model.sim_quantities[
                    "achievedAzimuth"
                ].last_val
                initial_actual_elevation = self.model.sim_quantities[
                    "achievedElevation"
    ...
    def test_long_running(self):
        with patch("time.sleep") as sleep_mock:
            ...
            self.assertEquals(
                num_method_calls,
                1,
                "The mocked method 'sleep' was called" " more than once.",
            )
            self.assertEquals(
                calls,
                sleep_mock.mock_calls,
                "The calls made to the" " mocked method 'sleep' are not the same.",
            )
...
class test_Device(ClassCleanupUnittestMixin, unittest.TestCase):
    ...
    def test_long_running(self):
        ...
        self.assertGreater(
            actual_runtime,
            long_running_cmd_min_exec_time,
            "The long-running command 'LongRun' did not run for as long" " as expected.",
        )

...
from tango_simlib.compat import ensure_byte_string
...
class XmiParser(Parser):
    ...
    def parse(self, sim_xmi_file):
        ...
        for child in tree.findall(".//"):
            for key, value in child.attrib.items():
                child.attrib[key] = ensure_byte_string(value)
    ...
    def get_device_attribute_metadata(self):
        ...
        for pogo_attribute_data in self._device_attributes:
            ...
            try:
                attributes[attribute_meta["name"]] = ensure_byte_string(attribute_meta)
            except TypeError:
